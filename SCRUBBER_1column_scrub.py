...
df['Local Count'] = df[firstcolumn].str.count('|'.join(local))
toc = time.perf_counter()
print(f"local done in {toc - tic:0.2f} seconds")

tic = time.perf_counter()
print("checking blacklist by comments...")
df['Blacklist'] = df[firstcolumn].str.count('|'.join(bl))
toc = time.perf_counter()
...
df['Score'] = 0 + (df['Pro Name']*3) - (df['Con Name']*2) - (df['Bad Name']*13) - (df['Blacklist']*139)
